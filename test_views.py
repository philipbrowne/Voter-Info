...
class ViewTestCase(TestCase):
    def setUp(self):
        ...
        db.session.commit()
        self.testuser = User.register(username='TEST_NAME', password='TEST_PW', first_name='TEST_FIRST', last_name='TEST_LAST',
                                      street_address='200 N Spring St', city='Los Angeles', county='Los Angeles', state_id='CA', zip_code='90012', email='test@test.com')
        db.session.commit()
        self.test_election = Election(
            name='Test Election', date='2022-01-01', state_id='CA')
        db.session.add(self.test_election)
        db.session.commit()
        self.test_rule = RegistrationRule(rule='TEST_RULE')
        db.session.add(self.test_rule)
        db.session.commit()
        self.test_state_reg_rule = StateRegistrationRule(
            state_id='CA', rule_id=self.test_rule.id)
